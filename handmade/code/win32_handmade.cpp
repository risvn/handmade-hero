...
// This macro defines a reusable DirectSound function signature.
// It allows us to declare functions with this signature using any name.
// The typedef below uses the macro to define a function *type* (not a function).
// This creates an alias 'direct_sound_create' for the function signature.
// We can now use it to declare function pointers (e.g., for GetProcAddress).
...
HMODULE WINAPI XInputLibrary = LoadLibrary("xinput1_4.dll"); 
if(!XInputLibrary)
  {
    HMODULE WINAPI XInputLibrary = LoadLibrary("xinput1_3.dll"); 
  }
    if(XInputLibrary)
  { 
...
Win32InitDSound(HWND Window,int32 SamplesPerSecond,int32 BufferSize)
{
  ...
  if(DSoundLibrary)
  {
    // Steps to load a dynamic function using a function pointer:
    // 1. Load the library (DLL/.so) at runtime
    // 2. Get the function address using GetProcAddress() / dlsym()
    // 3. Cast the address to the correct function pointer type
    // 4. Assign it to a function pointer variable
    // 5. Call the function through the pointer
    // 6. Unload the library when done (FreeLibrary() / dlclose())
  ...
  {
    //direct sound fails to load and diagnostic errors:
  }
